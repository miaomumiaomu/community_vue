        ...
        <div class="scroll">
          <ul>
            <li v-for="item in asyncData" :key="item.id" @click.stop="onItemChange(item)">
              <img :src="item.identityImg" class="pic">
              <div class="name">
                {{ item.name }}
                <div class="label-box">
...
export default {
  ...
  watch: {
    image(val) {
      let newImage = this.image.substr(this.image.indexOf(",") + 1);
      this.$emit("change", this.value, newImage);
    },
    value(val) {
      let newImage = this.image.substr(this.image.indexOf(",") + 1);
      this.$emit("change", this.value, newImage);
  ...
  methods: {
    ...
    async onSearchAsync(val, cb) {
      let _image = this.image.substr(this.image.indexOf(",") + 1);

      this.asyncData = [];
      this.$refs.input.focus();

      if (this.image.includes(".")) {
        _image = await getBase64(this.image);
        _image = _image.substr(_image.indexOf(",") + 1);
      }
