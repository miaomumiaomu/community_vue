  ...
  <div class="app-container">
    ...
    <el-table ref="listTabel" :data="listTableData.rows" height="calc(100vh - 255px)" v-loading.body="listTableLoading" highlight-current-row @selection-change="onListChange">
      ...
      <el-table-column fixed="right" width="130" align="center" label="操作">
        <template slot-scope="scope">
          <el-button size="mini" type="success" @click="onDetails(scope.row.id)" :plain="scope.row.handleFlag == 1" v-permission="'alarmMan:btn_alarm_detail'">{{scope.row.handleFlag == 1 ? '详情' : '处理'}}</el-button>
...
import { getMonitorTaskSelect } from "@/api/monitorTask";
import {
  postAlarmList,
  getAlarmDetail
} from "@/api/dataCenter/alarmNews/index";

export default {
  name: "alarmNews",
  components: {
    LabelForm: () =>
      import("@/views/faceModule/faceLabel/components/LabelForm"),
