<template>
  <div class="app-container calendar-list-container">
    ...
    <el-dialog title="派出所" :visible.sync="dialogFormVisible">
      <el-form :model="newForm" :rules="rules" ref="form" label-width="180px">
        <el-form-item label="上级" prop="xzqCode">
...
import { getObj, delObj } from "@/api/police/station";
export default {
  components: { SvgIcon },
  name: "district-policeStation",
  data() {
    return {
      currentNode: {},
      loading: false,
  ...
  methods: {
    fetchData() {
      this.loading = true;
      this.$store
        .dispatch("policeStation/query", this.filter)
        .then(payload => {
          this.loading = false;
    ...
    fetchTree() {
      this.$store.dispatch("policeStation/tree").then(payload => {
    ...
    onSearch() {
      this.loading = true;
      this.$store
        .dispatch("policeStation/query", { ...this.filter, offset: 1 })
        .then(payload => {
          this.loading = false;
          if (payload.status === 200) {
            this.list = payload.data.rows;
    ...
    handleDelete(id) {
      this.$confirm("此操作将永久删除, 是否继续?", "提示", {
        ...
        this.$store.dispatch("policeStation/remove", id).then(payload => {
          if (payload.status === 200) {
            this.fetchData();
          } else {
            this.$message({
    ...
    handleCreate(form) {
      this.$refs[form].validate(valid => {
        if (valid) {
          this.$store
            .dispatch("policeStation/create", this.newForm)
            .then(payload => {
              if (payload.status === 200) {
    ...
    handleUpdate(form) {
      ...
      this.$refs[form].validate(valid => {
        if (valid) {
          this.$store
            .dispatch("policeStation/patch", this.newForm)
            .then(payload => {
              if (payload.status === 200) {
                this.loading = false;
                this.handleCancel(form);
