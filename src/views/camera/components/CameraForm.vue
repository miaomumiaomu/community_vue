              ...
              <el-col :span="12">
                <el-form-item label="品牌" prop="cameraBrand">
                  <el-select v-model="form.cameraBrand" style="width: 100%">
                    <el-option v-for="item in cameraOptions" :key="item.brandCode" :label="item.brandName" :value="item.brandCode"/>
                  </el-select>
                </el-form-item>
              </el-col>
              <el-col :span="12">
...
import {getCameraBrandsCombo} from '@/api/cameraBrand'
...
export default {
  ...
  methods: {
    async initData() {
      ...
      await getCameraBrandsCombo({sort:'order_seq'}).then(res => {
        if (res.status == 200) {
