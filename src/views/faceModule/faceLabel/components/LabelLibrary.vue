<template>
  <el-col :span="5" class="label-library">
    <div class="title">
      <b>人脸库</b>
      <el-button size="mini" @click="openFormDialog()" v-permission="'faceLibraryMan:btn_add'">新增人脸库</el-button>
    </div>
      ...
      <li class="menu-item" v-for="(item,index) in menuData.labelStatisVOS" :key="item.num" :class="{sel: currentRow.num == item.num}" @click="onUpdataMenu(item,index)">
        ...
        <el-dropdown @command="onCommand($event, item)" class="more">
          ...
          <el-dropdown-menu slot="dropdown">
            <el-dropdown-item command="upd" v-permission="'faceLibraryMan:btn_edit'">编辑</el-dropdown-item>
            <el-dropdown-item command="del" v-permission="'faceLibraryMan:btn_delete'">删除</el-dropdown-item>
          </el-dropdown-menu>
        </el-dropdown>
      </li>
    </ul>
    ...
    <!-- 编辑人脸库弹层 S -->
    <el-dialog :title="form.id?'编辑人脸库':'新增人脸库'" class="form-layer" :visible.sync="isFormDialog" width="400px">
