      ...
      <el-col :span="19" class="right-box">
        ...
        <div class="common-tool">
          ...
          <div class="right">
            ...
            <div class="separator"></div>
            <el-button class="txt-btn" size="mini" type="text" icon="el-icon-upload" @click="onExcel" v-permission="'faceLabelMan:btn_download'">导出Excel</el-button>
            <div class="separator"></div>
            <el-upload class="upload-demo" :action="doUpload" accept=".xls, .xlsx" :file-list="fileList" style="display: -webkit-inline-box" :show-file-list="false" :before-upload="onBeforeUpload" :on-success="onUploadSuccess">
              <el-button class="txt-btn" size="mini" type="text" icon="el-icon-download" v-permission="'faceLabelMan:btn_download'">导入Excel</el-button>
            </el-upload>
            <div class="separator"></div>
        ...
        <el-table v-show="mode == 0" ref="listTabel" :data="listData.rows" height="calc(100vh - 255px)" v-loading.body="listLoading" highlight-current-row @selection-change="onListChange">
          ...
          <el-table-column prop="type" label="住户类型"></el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      fileList: [],
      doUpload:
        "/api/ai/label/import?Authorization=web458104a4534948b8ba9d7181d269354b"
    };
  },
  watch: {
    currentMenuData(val) {
  ...
  methods: {
    ...
    onItemDelete(type) {
      ...
      this.$confirm(`此操作将永久${tip}, 是否继续?`, "提示", {
        ...
        let excuter = deleteLabel;
        if (type != 4) excuter = cancelLabel;
        excuter({
          ids: this.listIds
        }).then(res => {
          let isSuccess = res.status === 200;
    ...
    onUploadSuccess(response, file, fileList) {
      postLabelImport(file).then(res => {
        this.getTableList();
      });
    },
    // 导出excel
    onExcel() {
      this.$confirm('此操作将导出所有信息，是否继续？', "提示", {
        confirmButtonText: '确认',
        confirmButtonClass: 'el-button--mini',
        cancelButtonText: '取消',
        cancelButtonClass: 'el-button--min fr',
        type: 'warning'
      }).then(() => {
        getExcel().then(res => {
          let isSuccess = res.status === 200;
