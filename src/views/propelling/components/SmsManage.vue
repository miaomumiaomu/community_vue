<template>
  <div class="sms-manage">
    ...
    <div class="common-tool">
      <div class="right">
        <el-button class="txt-btn" size="mini" type="text" icon="el-icon-plus" @click="onItemEdit()" v-permission="'smsPushMan:btn_add'">添加</el-button>
        <el-button class="txt-btn" size="mini" type="text" icon="el-icon-delete" @click="onItemDelete" v-permission="'smsPushMan:btn_batchDelete'">删除</el-button>
      </div>
    </div>
    ...
    <el-table class="table" :data="listData.rows" height="calc(100vh - 256px)" v-loading.body="listLoading" highlight-current-row @selection-change="onListChange">
      ...
      <el-table-column fixed="right" min-width="150" align="center" label="操作">
        <template slot-scope="scope">
          <el-button size="mini" type="primary" @click="onTest(scope.row)" v-permission="'smsPushMan:btn_test'">推送测试</el-button>
          <el-button size="mini" type="success" @click="onItemEdit(scope.row.id)" v-permission="'smsPushMan:btn_edit'">编辑</el-button>
        </template>
      </el-table-column>
    </el-table>
