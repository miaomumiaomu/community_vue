<template>
  <el-dialog title="布控详情" class="task-details body0" :visible.sync="isDetailsDialog" width="1100px">
    <el-row class="info">
      <el-col :span="8">
        <div class="item">
          <span>任务编号：</span>
    ...
    <el-table ref="listTabel" :data="listTableData.rows" height="calc(100vh - 520px)" v-loading.body="listTableLoading" highlight-current-row>
      ...
      <el-table-column label="抓拍照片" align="center" width="80">
        <template slot-scope="scope">
          <img-show class="pic" width="48px" height="48px" text="暂无照片" :img-src="scope.row.snapImg" />
        </template>
      </el-table-column>
      ...
      <el-table-column fixed="right" width="80" align="center" label="操作">
        <template slot-scope="scope">
          <el-button size="mini" type="success" @click="onAlarmDetails(scope.row.id)" :plain="scope.row.handleFlag == 1" v-permission="'alarmMan:btn_alarm_detail'">{{scope.row.handleFlag == 1 ? '详情' : '处理'}}</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 表格模式 E -->
...
import { getMonitorTask } from "@/api/monitorTask";
import { postAlarmList, getAlarmDetail } from "@/api/dataCenter/alarmNews";
...
export default {
  components: {
    ...
    swiperSlide,
    DisposeLayer: () =>
      import("@/views/dataCenter/alarmNews/components/DisposeLayer")
  },
  props: {
    id: Number,
    isOpen: {
