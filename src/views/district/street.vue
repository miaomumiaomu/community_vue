<template>
  <div>
    <el-row :gutter="20">
      <el-col :span="6">
        <div class="tree-container">
          ...
          <div class="labelList" v-for="data in treeData" :key="data.id">
      ...
      <el-col :span="18">
        ...
        <el-table :data="list" v-loading="loading" height="calc(100vh - 335px)" style="width: 100%" @selection-change="onListChange">
          ...
          <el-table-column prop="code" label="代码" min-width="100px">
          </el-table-column>
          <el-table-column prop="name" label="名称" min-width="100px">
          </el-table-column>
          <el-table-column prop="type" label="类型" min-width="60px">
          </el-table-column>
          <el-table-column prop="flag" label="启用" min-width="60px">
          </el-table-column>
    ...
    <el-dialog title="街路巷小区" :visible.sync="dialogFormVisible">
      <el-form :model="newForm" :rules="rules" ref="form" label-width="180px">
        <el-form-item label="上级" prop="sqjcwhCode">
          <span v-text="newForm.sqjcwhName"></span>
        </el-form-item>
        ...
        <el-form-item label="类型" prop="type">
          <el-select v-model="newForm.type" placeholder="请选择">
            <el-option label="街" value="1">
            </el-option>
            <el-option label="路" value="2">
            </el-option>
            <el-option label="巷" value="3">
            </el-option>
...
import { getObj, delObj } from "@/api/district/street";
export default {
  components: { SvgIcon },
  name: "street",
  data() {
  ...
  methods: {
    fetchData() {
      this.loading = true;
      this.$store
        .dispatch("street/query", this.filter)
        .then(payload => {
          this.loading = false;
    ...
    fetchTree() {
      this.$store.dispatch("street/tree").then(payload => {
        this.cachedTreeData = this.treeData = payload.data.rows;
      });
    },
    handleNodeClick(data, node) {
      this.filter.sqjcwhCode = data.code;
      this.fetchData();
    },
    filterList(keywords) {
      if (keywords) {
    ...
    onSearch() {
      this.loading = true;
      this.$store
        .dispatch("street/query", { ...this.filter, offset: 1 })
        .then(payload => {
          this.loading = false;
          if (payload.status === 200) {
            this.list = payload.data.rows;
    ...
    createDialog(data) {
      this.newForm = Object.assign({}, this.newForm, {
        sqjcwhCode: data.code,
        sqjcwhName: data.label
      });
      this.dialogFormVisible = true;
    },
    handleDelete(id) {
      this.$confirm("此操作将永久删除, 是否继续?", "提示", {
        ...
        this.$store.dispatch("street/remove", id).then(payload => {
          if (payload.status === 200) {
            this.fetchData();
          } else {
            this.$message({
    ...
    handleCreate(form) {
      this.$refs[form].validate(valid => {
        if (valid) {
          this.$store
            .dispatch("street/create", this.newForm)
            .then(payload => {
              if (payload.status === 200) {
    ...
    handleUpdate(form) {
      ...
      this.$refs[form].validate(valid => {
        if (valid) {
          this.$store
            .dispatch("street/patch", this.newForm)
            .then(payload => {
              if (payload.status === 200) {
                this.loading = false;
                this.handleCancel(form);
