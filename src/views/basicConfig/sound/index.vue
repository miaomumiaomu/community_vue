  ...
  <div class="sound-container">
    <el-row>
      ...
      <el-col :span="5" class="tree-container">
        <div class="title">
          <el-button size="mini" @click="onVoiceUpdate('')" v-permission="'voiceMan:btn_add'" icon="el-icon-plus">添加配置</el-button>
        </div>
        <div class="list">
          <div class="item" :class="{sel:templetQuery.voiceId == item.id}" @click="templetQuery.voiceId = item.id" v-for="item in voiceList" :key="item.id">
            ...
            <el-dropdown @command="onCommand($event, item)" class="more">
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item command="upd" v-permission="'voiceMan:btn_edit'">编辑</el-dropdown-item>
                <el-dropdown-item command="del" v-permission="'voiceMan:btn_delete'">删除</el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown>
          </div>
        </div>
      ...
      <el-col :span="19">
        ...
        <div class="common-tool">
          ...
          <div class="right">
            <el-button class="txt-btn" size="mini" icon="el-icon-plus" type="text" @click="onTempletUpdate('')" v-permission="'voiceTempletMan:btn_add'">添加</el-button>
            <el-button class="txt-btn" size="mini" icon="el-icon-delete" type="text" @click="onItemDelete" style="margin-left:0px" v-permission="'voiceTempletMan:btn_delete'">删除</el-button>
          </div>
        </div>
        <el-table :data="voiceTempletList.rows" v-loading="tableLoading" height="calc(100vh - 306px)" @selection-change="onListChange">
          ...
          <el-table-column align="center" label="操作" width="130" >
            <template slot-scope="scope">
              <el-button type="success" size="mini" v-permission="'voiceTempletMan:btn_edit'" @click="onTempletUpdate(scope.row)">编辑</el-button>
              <audio-button :src="scope.row.url" />
            </template>
          </el-table-column>
        </el-table>
    ...
    <!-- 语音配置弹窗 S -->
    <el-dialog :title="addTempletQuery.id === null?'新增语音模板配置':'编辑语音模板配置'" class="template-layer" :visible.sync="isTempletDialog" width="500px">
      <el-form :model="addTempletQuery" ref="templetForm" label-width="60px" :rules="templetRules">
